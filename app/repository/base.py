...
class BaseRepository:
    ...
    @classmethod
    async def find_by_id(cls, record_id: str, includes: List[str] = None):
        async with async_session_maker() as session:
            query = select(cls)
            if includes:
                for include in includes:
    ...
    @classmethod
    async def find_by_id_or_fail(cls, record_id: int, includes: List[str] = None):
        async with async_session_maker() as session:
            query = select(cls)
            if includes:
                for include in includes:
    ...
    @classmethod
    async def get_or_create(cls, filter, includes: List[str] = None, **data):
        async with async_session_maker() as session:
